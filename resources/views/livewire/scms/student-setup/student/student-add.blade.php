<div>
    ...
    <x-modal wire:model="admissionNumberingModal" class="backdrop-blur text-xs" box-class="w-100" persistent>
        <x-select class="w-full!" label="{{ __('Admission Numbering') }}" :options="$documentNumberings" option-label="admission_no"
        ...
        Alpine.store('studentSetup', {
            ...
            saveStudent() {
                const data = {
                    ...
                    documentForm: $store.studentSetup.documentForm ?? [],
                };
            ...
            removeDocumentRow(index) {
                console.log(index)
                $wire.call('removeDocument', index)
            ...
            uploadDocument(event, index) {
                ...
                if (!file) return;
                this.documentForm[index].file = file;
                this.documentForm[index].preview = URL.createObjectURL(file);
                $wire.upload('documents.' + index + '.file', file);
            },

            removeDocumentFile(index) {
                this.documentForm[index].file = null
                this.documentForm[index].preview = null
                $wire.set('documents.' + index + '.file', null)
            },
