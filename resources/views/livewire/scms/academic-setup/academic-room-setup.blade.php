        ...
        <x-table class="text-xs" :headers="$headers" :rows="$room_data_list" :sort-by="$sortBy" with-pagination>
            ...
            <div class="flex text-xs">
                <div x-cloak x-show="allowedPermissions.edit">
                    <x-button icon="o-pencil" spinner="edit({{ $room_data->id }})"
        ...
        <x-card separator progress-indicator="saveAcademicRoom">
            <x-form no-separator wire:submit="saveAcademicRoom"
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <x-input type="number" label="{{ __('Enter Floor Number') }}:"
                             placeholder="{{ __('Enter Floor Number') }}"
                             wire:model='roomForm.floor_no'/>

                    <x-input type="number" label="{{ __('Enter Block Number') }}:"
                             placeholder="{{ __('Enter Block Number') }}"
