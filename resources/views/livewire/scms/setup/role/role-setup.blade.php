<div x-data="{ drawer: @entangle('drawer'), allowedPermissions: @js($allowedPermissions) }">
            ...
            <div class="flex text-sm">
                <div x-cloak x-show="allowedPermissions.edit">
                    <x-button icon="o-pencil" class="btn-ghost btn-xs text-indigo-500" tooltip-bottom="{{ __('Edit') }}"
                              link="{{ route('setup.role.create', ['id' => $role->id]) }}"/>
                </div>
                ...
                <div x-cloak x-show="allowedPermissions.delete">
                    <x-button icon="o-trash" spinner=" " class="btn-ghost btn-xs text-red-500"
                              ...
                              @click.prevent="$store.roleSetup.deleteData({{$role['id']}}); $wire.drawer = true"/>
                </div>
    ...
    <x-modal wire:model="drawer" title="{{ __('Are you sure?') }}" class="backdrop-blur" box-class="w-120">
        <x-card separator progress-indicator="deleteRole">
            ...
            <x-slot:actions>
                <x-button label="{{ __(key: 'Cancel') }}"
                          @click.prevent="$wire.drawer = false; $store.roleSetup.resetDeleteData()"
                          class="btn-xs p-3.5"/>
                <x-button label="{{ __('Delete') }}" spinner="delete"
                          @click.prevent="$wire.delete($store.roleSetup.role_id)" class="btn-error btn-xs p-3.5"/>
            </x-slot:actions>
        </x-card>
