    ...
    <x-card separator progress-indicator="saveAcademicTimetable">
        <x-form no-separator @submit.prevent="$store.academicTimetableSetup.saveAcademicTimetable"
            ...
            <div class="grid grid-cols-5 gap-3">
                <div>
                    <x-input label="{{ __('Enter Timetable Name') }}:" placeholder="{{ __('Enter Timetable name') }}"
                        x-model="$store.academicTimetableSetup.timetableData.name" />
                    <span x-text="$store.academicTimetableSetup.errors?.name || ''" class="text-red-500 text-xs"></span>
                </div>
                ...
                <x-select label="{{ __('Status') }}:" x-model="$store.academicTimetableSetup.timetableData.status"
                    :options="$status" option-value="value" option-label="label" />
                <template x-if="$store.academicTimetableSetup.errors?.status">
                    <small class="text-red-500" x-text="$store.academicTimetableSetup.errors.status"></small>
                </template>
            </div>
                    ...
                    <table class="min-w-full border-2 border-gray-400 border-collapse">
                        ...
                        <thead class="bg-emerald-600 text-white rounded-md">
                            <tr>
                                ...
                                <th class="px-4 py-2 border-2 border-gray-400 text-left">Type</th>
                                <th class="px-4 py-2 border-2 border-gray-400 text-left">Period No.</th>
                                <th class="px-4 py-2 border-2 border-gray-400 text-left">Subject</th>
                                <th class="px-4 py-2 border-2 border-gray-400 text-left">Teacher</th>
                        ...
                        <tbody class="bg-white">
                            <template x-for="(detail, index) in $store.academicTimetableSetup.timetable_detail"
                                :key="index">
                                <tr class="hover:bg-emerald-50 transition">
                                    <td class="px-4 py-2 border-2 border-gray-400" x-text="index + 1"></td>
                                    <td class="px-4 py-2 border-2 border-gray-400">
                                        <x-select x-model="$store.academicTimetableSetup.timetable_detail.type"
                                            :options="$type" option-value="value" option-label="label" />
                                        <template x-if="$store.academicTimetableSetup.errors?.type">
                                            <small class="text-red-500"
                                                x-text="$store.academicTimetableSetup.errors.type"></small>
                                        </template>
                                    </td>
                                    
                                    <td class="px-4 py-2 border-2 border-gray-400">
                                        <x-input placeholder="{{ __('Enter Period Number') }}"
                                            x-model='$store.academicTimetableSetup.timetable_detail.period_no' />
                                    </td>

                                    <td class="px-4 py-2 border-2 border-gray-400">
                                        <div class="flex flex-col w-full" wire:ignore>
                                            <select class="academic-structure-select"
                                                @change.prevent="$store.academicTimetableSetup.updateSelectedData('subject_id',$event.target.value)"
                                                x-bind:id="'subject_id'" x-bind:data-row-index="'subject_id'"
                                                x-model="$store.academicTimetableSetup.timetable_detail.subject_id">
                                                <option value="">{{ 'Add Program' }}</option>
                                                <template
                                                    x-for="listItem in $store.academicTimetableSetup.academicStructures.map(listItem => JSON.parse(JSON.stringify(listItem)))"
                                                    :key="listItem.id">
                                                    <option :value="listItem.id" x-text="listItem.text"
                                                        :selected="listItem.id === $store.academicTimetableSetup?.timetableData
                                                            ?.subject_id ||
                                                            null">
                                                    </option>
                                                </template>
                                            ...
                                            <span x-text="$store.academicTimetableSetup.errors?.subject_id || ''"
                                                class="text-red-500 text-xs"></span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-2 border-2 border-gray-400 w-30">
                                        <x-button icon="o-plus" @click.prevent="$store.academicTimetableSetup.addRow()"
                                            class="btn-xs btn-success text-white" />
                                        <template x-if="index > 0">
                                            <x-button icon="o-minus"
                                                @click.prevent="$store.academicTimetableSetup.removeRow(index)"
...
@script
    <script>
        Alpine.store('academicTimetableSetup', {
            ...
            removeRow(index) {
                this.timetable_detail.splice(index, 1)
            },
            ...
            updateSelectedData(key, value) {
                ...
                if (key == 'structure_id') {
                    ...
                    $wire.fetchStructureData(value).then((response) => {
                        ...
                        this.structureData.academic_level = response.structure.academic_level.charAt(0)
                            .toUpperCase() + response.structure.academic_level.slice(1).toLowerCase();
                        ...
                        this.structureData.academic_system = response.structure.academic_system.charAt(0)
                            .toUpperCase() + response.structure.academic_system.slice(1).toLowerCase();
                        this.loading = false;
